...
ex=pd.Series(np.arange(5.),index=['a','b','c','d','e'])
ex
...
"""slicing with lables is difenent i.e end point is inclusive"""
...
exdf=pd.DataFrame(np.arange(9).reshape(3,3),index=['aa','bb','cc'],columns=['a','b','c'])
exdf

"""for column selection we have to pass a single elements or a list to [ ]"""

exdf['b']

exdf[['a','b']]

"""row selection syntax is df[:]"""
...
"""we can do similar with iloc using integers"""
...
"""we cannot use [-1] to get last value in integer indexing"""

se[-1]

"""we can do it for non integer indexing"""

se2=pd.Series(np.arange(3.),index=['a','b','c'])
se2

se2[-1]

"""for precise handling using loc for lables and iloc for integers"""
